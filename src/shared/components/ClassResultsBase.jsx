...
const ClassResultsBase = ({ useClassHook }) => {
  const [selectedExam, setSelectedExam] = useState(null);
  const { results, fetchResults, loadingResults } = useClassHook();
  ...
  const notaMedia = (
    results.reduce((sum, exam) => sum + exam.nota, 0) / results.length
  ).toFixed(1);

  const tiempoMedio = Math.round(
    results.reduce((sum, exam) => sum + exam.tiempo, 0) / results.length
  );
    ...
    >
      ...
      <motion.div variants={itemVariants} className="lg:col-span-1 space-y-4">
        ...
        <div className="bg-white dark:bg-gray-800 border-gray-200 dark:border-gray-700 rounded-xl p-6 border shadow-lg">
          <h2 className="text-lg text-purple-700 dark:text-purple-300 mb-2 font-medium">
            Nota Media
          </h2>
        ...
        <div className="bg-white dark:bg-gray-800 border-gray-200 dark:border-gray-700 rounded-xl p-6 px-4 border shadow-lg">
          <h2 className="text-lg text-purple-700 dark:text-purple-300 mb-4 font-medium">
            Últimos Exámenes
          </h2>
      ...
      <motion.div variants={itemVariants} className="lg:col-span-2 space-y-6">
        ...
        <div className="bg-white dark:bg-gray-800 border-gray-200 dark:border-gray-700 rounded-xl p-6 border shadow-lg">
          <div className="flex items-center mb-4">
            ...
            <h2 className="text-lg text-purple-700 dark:text-purple-300 font-medium">
              Evolución de Notas
            </h2>
          </div>
          <div className="h-64">
        ...
        <div className="bg-white dark:bg-gray-800 border-gray-200 dark:border-gray-700 rounded-xl p-6 border shadow-lg">
          ...
          <div className="flex flex-col md:flex-row gap-6">
            <motion.div whileHover={{ scale: 1.05 }} className="bg-gray-100 dark:bg-gray-700/50 border-gray-200 dark:border-gray-700 rounded-lg p-4 flex-1 border">
            ...
            <motion.div whileHover={{ scale: 1.05 }} className="bg-gray-100 dark:bg-gray-700/50 border-gray-200 dark:border-gray-700 rounded-lg p-4 flex-1 border">
              <div className="text-3xl font-bold text-purple-600 dark:text-purple-400">{Math.max(...results.map((e) => e.nota))}</div>
...
export default ClassResultsBase;
