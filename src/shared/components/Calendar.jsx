        ...
        <div className="grid grid-cols-7 text-center text-sm gap-y-1">
          ...
          {[...Array(daysInMonth)].map((_, i) => {
            ...
            return (
              <motion.div
                key={day}
                whileHover={hasExams ? { scale: 1.15, rotate: [0, 1, -1, 0] } : {}}
              ...
              >
                <div
                  ...
                  ${hasExams ? `ring-2 ${ringColor}` : ""}
                `}
                >
                  {day}
                  {isToday(dateStr) && !hasExams && (
                    <span className="absolute -top-1.5 -right-1.5 bg-purple-500 text-white text-[10px] px-1 rounded-full shadow">
                    Hoy
                  </span>
                  )}
                </div>
                ...
                {hasExams && (
                  <div
                    className="absolute -top-10 left-1/2 -translate-x-1/2 w-max max-w-[160px] px-3 py-1 text-xs rounded-xl shadow-lg opacity-0 group-hover:opacity-100 pointer-events-none z-10 transition-all duration-300 bg-white/90 dark:bg-gray-900/80 backdrop-blur text-gray-800 dark:text-white transform translate-y-1 group-hover:translate-y-0"
                  >
                    {exams.map((exam, idx) => (
                      <p key={idx} className="leading-tight">{exam.title}</p>
                    ))}
                  </div>
                )}
              </motion.div>
